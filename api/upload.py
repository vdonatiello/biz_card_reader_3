...
def handler(request):
    """
    Vercel Python handler function for 2025
    Uses the simple function format that works with current Vercel runtime
    """
    ...
    try:
        # Handle different ways Vercel might pass form data
        image_data = None
        ...
        if hasattr(request, 'files') and request.files:
            ...
            # Form data with base64
            form_keys = list(request.form.keys())
            if form_keys:
                file_data = request.form[form_keys[0]]
            ...
            if isinstance(request_data, bytes):
                # Try to parse as multipart or JSON
                try:
                    # Check if it's JSON
                    json_data = json.loads(request_data.decode())
                    ...
                    # Assume it's multipart form data
                    if b'Content-Type: image' in request_data:
                        # Simple extraction of image data from multipart
                        parts = request_data.split(b'\r\n\r\n')
                        for part in parts:
                            if len(part) > 1000:  # Likely the image data
                                # Remove potential boundary markers
                                lines = part.split(b'\r\n')
                                image_data = lines[0]
                                break
        ...
        result = process_business_card(image_data)
...
def process_business_card(image_data):
    """Process business card image with OpenAI GPT-4o"""
    try:
